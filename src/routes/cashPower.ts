...
import { CashPowerController } from '../controller/cashPowerController';
...
// Get monthly trends
router.get('/trends/monthly', async (c) => {
  try {
    const monthlyTrends = await controller.getMonthlyTrends(c);
    return c.json(monthlyTrends);
  } catch (error: unknown) {
    if (error instanceof Error) {
      return c.json({ error: error.message }, 500);
    }
