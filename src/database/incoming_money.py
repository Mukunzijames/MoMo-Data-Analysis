...
def parse_transaction_details(body):
    ...
    if "from" in body:
        recipient_part = body.split("from")[-1]
        recipient = recipient_part.split("on")[0].strip().split("(")[0].strip()
...
def main():
    ...
    save_to_json(transactions, "incoming_money.json")
    print("✅ Incoming money transactions processed and saved to 'incoming_money.json'.")
